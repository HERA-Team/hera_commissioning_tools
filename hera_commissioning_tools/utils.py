    ...
    for n in sorted(inclNodes):
        ...
        for ant in nodes[n]['ants']:
            if len(pols) == 2:
                ...
                antnum = ant
            if use_ants != 'all' and int(antnum) not in use_ants and ant not in use_ants:
                continue
            for pol in pols:
        ...
        for loc in locs:
            ...
            for ant in ants:
                if len(pols) == 2:
                    ...
                    key = 'HH%i:A' % ant
                    pol = pols[0]
                pair = (int(x[key].hookup[f'{pol}<ground'][-2].downstream_input_port[1:]), ant)
...
def get_baseline_groups(uv, bl_groups=[(14, 0, '14m E-W'), (29, 0, '29m E-W'), (14, -11, '14m NW-SE'),
                                       (14, 11, '14m SW-NE')],
                        use_ants='all'):
    """
...
def gather_source_list(catalog_path='G4Jy_catalog.tsv'):
    ...
    sources = [(50.6750, -37.2083, 'Fornax A'), (201.3667, -43.0192, 'Cen A'), (252.7833, 4.9925, 'Hercules A'),
               (139.5250, -12.0947, 'Hydra A'), (79.9583, -45.7789, 'Pictor A'), (187.7042, 12.3911, 'Virgo A'),
               (83.8208, -59.3897, 'Orion A'), (80.8958, -69.7561, 'LMC'), (13.1875, -72.8286, 'SMC'),
               (201.3667, -43.0192, 'Cen A'), (83.6333, 20.0144, 'Crab Pulsar'), (128.8375, -45.1764, 'Vela SNR')]
